...
This page is a human-readable companion to the [OpenAPI specification](./datahub-api.yaml). Use it to understand what each endpoint does before diving into the full schema.
...
*See the full [OpenAPI specification](./datahub-api.yaml) for complete request/response schemas.*